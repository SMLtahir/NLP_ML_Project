...
/**
 * Meta-collector for {@link T_NGramFeatureExtractor}.
 * 
 * Adopted from DKPro TC
 * @version last updated: Apr 03, 2014 [Sousa]
 */
public class T_NGramMetaCollector
    extends FreqDistBasedMetaCollector
{
    public static final String NGRAM_FD_KEY = "T_ngrams.ser";

    @ConfigurationParameter(name = T_NGramFeatureExtractor.PARAM_NGRAM_FD_FILE, mandatory = true)
    private File teacherNgramFdFile;
    ...
    {
        FrequencyDistribution<String> teacherNGrams = NGramUtils.getTeacherNgrams(
                jcas, ngramLowerCase, filterPartialStopwordMatches, ngramMinN, ngramMaxN, stopwords);      

        for (String ngram : teacherNGrams.getKeys()) {
            fd.addSample(ngram, teacherNGrams.getCount(ngram));
        }
    }
    ...
    public Map<String, String> getParameterKeyPairs()
    {
        ...
        mapping.put(T_NGramFeatureExtractor.PARAM_NGRAM_FD_FILE, NGRAM_FD_KEY);
        return mapping;
    }
