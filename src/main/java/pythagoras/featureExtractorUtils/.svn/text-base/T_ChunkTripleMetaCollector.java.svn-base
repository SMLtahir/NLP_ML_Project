...
/**
 * Meta-collector for {@link T_ChunkTripleFeatureExtractor}.
 * 
 * Adopted from DKPro TC
 * @version last updated: Apr 03, 2014 [Sousa]
 */
public class T_ChunkTripleMetaCollector
    extends FreqDistBasedMetaCollector
{
    public static final String CHUNK_TRIPLE_FD_KEY = "T_chunkTriple.ser";

    @ConfigurationParameter(name = T_ChunkTripleFeatureExtractor.PARAM_CHUNK_TRIPLE_FD_FILE, mandatory = true)
    private File teacherChunkTripleFdFile;

    @ConfigurationParameter(name = T_ChunkTripleFeatureExtractor.PARAM_CHUNK_TRIPLE_LOWER_CASE, mandatory = false, defaultValue = "true")
    private boolean chunkTripleLowerCase;

    @ConfigurationParameter(name = T_ChunkTripleFeatureExtractor.PARAM_CHUNK_TRIPLE_STOPWORDS_FILE, mandatory = false)
...
Set<String> triples = new HashSet<String>();
    ...
    private static String getTriple(JCas jcas, Chunk vc, Set<String> stopwords)
    {
        ...
        if (ncListLeft.size()>0) { ncStringLeft = getChunkString(jcas, ncListLeft.subList(0, 1)); } else ncStringLeft = getChunkString(jcas, ncListLeft);
        if (ncListRight.size()>0) { ncStringRight = getChunkString(jcas, ncListRight.subList(0, 1)); } else ncStringRight = getChunkString(jcas, ncListRight);
    ...
    public Map<String, String> getParameterKeyPairs()
    {
        ...
        mapping.put(T_ChunkTripleFeatureExtractor.PARAM_CHUNK_TRIPLE_FD_FILE, CHUNK_TRIPLE_FD_KEY);
        return mapping;
    }
    ...
    {
        return teacherChunkTripleFdFile;
    }
}