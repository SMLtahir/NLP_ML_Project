...
/**
 * Measures the positivity/negativity of student speech while dividing words into strong, weak and neutral sentiment types.
 ...
 */
public class S_SentimentPolarityWordsNumberFeatureExtractor
    ...
    implements DocumentFeatureExtractor
{
    ...
    public List<Feature> extract(JCas view) throws TextClassificationException
    {
        ...
        featList.addAll(Arrays.asList(getPolarityFeature(GROUP_PREFIX+ "SentimentOfWords_S", totalPosWordsCount, totalNegWordsCount)));
        featList.addAll(Arrays.asList(getPolarityFeature(GROUP_PREFIX+ "SentimentOfStrongWords_S", totalStrongPosWordsCount, totalStrongNegWordsCount)));
        featList.addAll(Arrays.asList(getPolarityFeature(GROUP_PREFIX+ "SentimentOfWeakWords_S", totalWeakPosWordsCount, totalWeakNegWordsCount)));
        featList.add(new Feature(GROUP_PREFIX+ "StrongPosWords_S", totalSentiWordsCount == 0 ? 0 : totalStrongPosWordsCount/ totalSentiWordsCount));
        featList.add(new Feature(GROUP_PREFIX+ "StrongNegWords_S", totalSentiWordsCount == 0 ? 0 : totalStrongNegWordsCount/ totalSentiWordsCount));
        featList.add(new Feature(GROUP_PREFIX+ "WeakPosWords_S", totalSentiWordsCount == 0 ? 0 : totalWeakPosWordsCount/ totalSentiWordsCount));
        featList.add(new Feature(GROUP_PREFIX+ "WeakNegWords_S", totalSentiWordsCount == 0 ? 0 : totalWeakNegWordsCount/ totalSentiWordsCount));
        return featList;
    }
