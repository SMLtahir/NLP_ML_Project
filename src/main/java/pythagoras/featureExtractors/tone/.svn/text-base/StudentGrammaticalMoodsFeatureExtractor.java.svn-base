 ...
 * 
 * This feature extractor extracts the student's mood by this grammatical logic.
 * @author Tahir Sousa
 * @version last updated: Jun 26, 2014 [Sousa]
 */
public class StudentGrammaticalMoodsFeatureExtractor 
    ...
    implements DocumentFeatureExtractor
{
    ...
    private static final String GROUP_PREFIX = "SE_Mood__";
    
    @Override
    public List<Feature> extract(JCas jcas) throws TextClassificationException 

    {
    	//TODO: Additionally, introduce detection of words like "must" or "have to" to detect commands orders
        ...
        featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ VIMP_PER_VERB + "_S", nrOfVerbs!=0? nrImperativVerbs / nrOfVerbs: 0)));
        featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ VKONJ_PER_VERB + "_S", nrOfVerbs!=0? nrKonjunctivVerbs / nrOfVerbs: 0)));
        featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ VIND_PER_VERB + "_S", nrOfVerbs!=0? nrIndicativVerbs / nrOfVerbs: 0)));
        
        featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ VIMP_PER_CONV + "_S", nrImperativVerbs / nrOfConversations)));
        featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ VKONJ_PER_CONV + "_S", nrKonjunctivVerbs / nrOfConversations)));
        featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ VIND_PER_CONV + "_S", nrIndicativVerbs / nrOfConversations)));
