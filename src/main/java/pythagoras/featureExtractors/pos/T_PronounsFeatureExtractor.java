...
/**
 * Feature extractor based on the frequency of top-occurring pronouns in teacher utterances
 * 
 * @author Tahir Sousa
    ...
    implements MetaDependent, DocumentFeatureExtractor
{
        ...
        throws TextClassificationException
    {
        ...
        for (String topPr : topPronouns) {
            if (features.contains(topPr)) {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_pr_Bool_" + topPr, 1)));
            }
            else {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_pr_Bool_" + topPr, 0)));
            }
        }
        ...
        featList.add(new Feature(GROUP_PREFIX+ "CosineSimilarityTopPronouns_T", cosineScore/nrOfLessonPronouns));
        return featList;
    }
    ...
    private List<Feature> countPronounHits(String wordListName, List<String> pr)
            throws TextClassificationException
        {
            ...
            return Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_pr_ratio_" + wordListName,
