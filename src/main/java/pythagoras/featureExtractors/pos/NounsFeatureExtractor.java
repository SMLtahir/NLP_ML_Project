...
/**
 * Feature extractor based on the frequency of top-occurring nouns in all utterances
 * 
 * @author Tahir Sousa
    ...
    implements MetaDependent, DocumentFeatureExtractor
{
        ...
        throws TextClassificationException
    {
    	...
    	cleanedNouns = NGramUtils.removeSpecialCharacters(features);
        try {
        ...
        for (String topNn : topNouns) {
            if (cleanedNouns.contains(topNn)) {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "feature_nn_Bool_" + topNn, 1)));
            }
            else {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "feature_nn_Bool_" + topNn, 0)));
            }
        }
      //Measure Cosine Similarity score between the top nouns of the corpus and those of the lesson.
        ...
        featList.add(new Feature(GROUP_PREFIX+ "CosineSimilarityTopNouns_Gen", cosineScore/nrOfLessonNouns));
        return featList;
    }
    ...
    private List<Feature> countNounHits(String wordListName, List<String> nn)
            throws TextClassificationException
        {
            ...
            return Arrays.asList(new Feature(GROUP_PREFIX+ "feature_nn_ratio_" + wordListName,
