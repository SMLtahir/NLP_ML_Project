...
import pythagoras.featureExtractorUtils.T_AdverbsMetaCollector;
...
/**
 * Feature extractor based on the frequency of top-occurring adverbs in teacher utterances
 * 
 * @author Tahir Sousa
 * @version last updated: Apr 22, 2014 [Sousa]
 */
public class T_AdverbsFeatureExtractor
    extends FeatureExtractorResource_ImplBase
    implements MetaDependent, DocumentFeatureExtractor
{

    public static final String PARAM_T_ADVERBS_FD_FILE = "TeacherAdverbsFdFile";
    public static final String PARAM_USE_TOP_K_ADVERBS = "TopKAdverbs";

    @ConfigurationParameter(name = PARAM_T_ADVERBS_FD_FILE, mandatory = true)
    private String teacherAdverbsFdFile;
        ...
        throws TextClassificationException
    {
        ...
        for (String topAdv : topAdverbs) {
            if (features.contains(topAdv)) {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_adv_Bool_" + topAdv, 1)));
            }
            else {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_adv_Bool_" + topAdv, 0)));
            }
        }
      //Measure Cosine Similarity score between the top Adverbs of the corpus and those of the lesson.
        ...
        featList.add(new Feature(GROUP_PREFIX+ "CosineSimilarityTopAdverbs_T", cosineScore/nrOfLessonAdverbs));
        return featList;
    }
    ...
    private List<Feature> countAdverbHits(String wordListName, List<String> adv)
            throws TextClassificationException
        {
            ...
            return Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_adv_ratio_" + wordListName,
        ...
        throws ResourceInitializationException
    {
        try {
            ...
            trainingFD.load(new File(teacherAdverbsFdFile));
    ...
    public List<Class<? extends MetaCollector>> getMetaCollectorClasses()
    {
        ...
        metaCollectorClasses.add(T_AdverbsMetaCollector.class);
