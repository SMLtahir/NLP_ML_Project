...
/**
 * Feature extractor based on the frequency of top-occurring verbs in all utterances
 * 
 * @author Tahir Sousa
    ...
    implements MetaDependent, DocumentFeatureExtractor
{
        ...
        throws TextClassificationException
    {
    	...
    	cleanedVerbs = NGramUtils.removeSpecialCharacters(features);
        try {
        ...
        for (String topV : topVerbs) {
            if (cleanedVerbs.contains(topV)) {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "feature_verb_Bool_" + topV, 1)));
            }
            else {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "feature_verb_Bool_" + topV, 0)));
            }
        }
      //Measure Cosine Similarity score between the top Verbs of the corpus and those of the lesson.
        ...
        featList.add(new Feature(GROUP_PREFIX+ "CosineSimilarityTopVerbs_Gen", cosineScore/nrOfLessonVerbs));
        return featList;
    }
    ...
    private List<Feature> countVerbHits(String wordListName, List<String> v)
            throws TextClassificationException
        {
            ...
            return Arrays.asList(new Feature(GROUP_PREFIX+ "feature_v_ratio_" + wordListName,
