...
/**
 * Feature extractor based on the frequency of top-occurring adjectives in all utterances
 * 
 * @author Tahir Sousa
    ...
    implements MetaDependent, DocumentFeatureExtractor
{
        ...
        throws TextClassificationException
    {
    	...
    	cleanedAdjectives = NGramUtils.removeSpecialCharacters(features);
        try {
            subgroups = FileUtils.readLines(new File(adjectiveSubgroupsFilePath));
            for (String group : subgroups) {
                featList.addAll(countAdjectiveHits(group, cleanedAdjectives));
            }
        }
        catch (IOException e) {
            e.printStackTrace();
        ...
        for (String topAdj : topAdjectives) {
            if (cleanedAdjectives.contains(topAdj)) {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "feature_adj_Bool_" + topAdj, 1)));
            }
            else {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "feature_adj_Bool_" + topAdj, 0)));
            }
        }
        ...
        featList.add(new Feature(GROUP_PREFIX+ "CosineSimilarityTopAdjectives_Gen", cosineScore/nrOfLessonAdjectives));
        return featList;
    }
    ...
    private List<Feature> countAdjectiveHits(String wordListName, List<String> adj)
            throws TextClassificationException
        {
            ...
            return Arrays.asList(new Feature(GROUP_PREFIX+ "feature_adj_ratio_" + wordListName,
