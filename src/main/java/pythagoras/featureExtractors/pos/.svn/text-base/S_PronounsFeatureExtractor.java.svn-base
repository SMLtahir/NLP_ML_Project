...
import pythagoras.featureExtractorUtils.S_PronounsMetaCollector;
...
/**
 * Feature extractor based on the frequency of top-occurring pronouns in student utterances
 * 
 * @author Tahir Sousa
 * @version last updated: Apr 22, 2014 [Sousa]
 */
public class S_PronounsFeatureExtractor
    extends FeatureExtractorResource_ImplBase
    implements MetaDependent, DocumentFeatureExtractor
{

    public static final String PARAM_S_PRONOUNS_FD_FILE = "StudentPronounsFdFile";
    public static final String PARAM_USE_TOP_K_PRONOUNS = "TopKPronouns";

    @ConfigurationParameter(name = PARAM_S_PRONOUNS_FD_FILE, mandatory = true)
    private String studentPronounsFdFile;
        ...
        throws TextClassificationException
    {
        ...
        for	(TranscribedText tt : JCasUtil.select(jcas, TranscribedText.class))	{
    		if	(tt.getSpeaker().contains("S"))	{
    			...
    			for	(Token t : tokens)	{
    				...
    				if	(!pos.startsWith("PTK") && pos.startsWith("P"))	{
        ...
        for (String topPr : topPronouns) {
            if (features.contains(topPr)) {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "S_feature_pr_Bool_" + topPr, 1)));
            }
            else {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "S_feature_pr_Bool_" + topPr, 0)));
            }
        }
        ...
        featList.add(new Feature(GROUP_PREFIX+ "CosineSimilarityTopPronouns_S", cosineScore/nrOfLessonPronouns));
        return featList;
    }
    ...
    private List<Feature> countPronounHits(String wordListName, List<String> pr)
            throws TextClassificationException
        {
            ...
            return Arrays.asList(new Feature(GROUP_PREFIX+ "S_feature_pr_ratio_" + wordListName,
        ...
        throws ResourceInitializationException
    {
        try {
            ...
            trainingFD.load(new File(studentPronounsFdFile));
    ...
    public List<Class<? extends MetaCollector>> getMetaCollectorClasses()
    {
        ...
        metaCollectorClasses.add(S_PronounsMetaCollector.class);
