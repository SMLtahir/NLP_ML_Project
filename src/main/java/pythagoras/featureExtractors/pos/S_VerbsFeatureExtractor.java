...
import pythagoras.featureExtractorUtils.S_VerbsMetaCollector;
...
/**
 * Feature extractor based on the frequency of top-occurring verbs in student utterances
 * 
 * @author Tahir Sousa
 * @version last updated: Apr 22, 2014 [Sousa]
 */
public class S_VerbsFeatureExtractor
    extends FeatureExtractorResource_ImplBase
    implements MetaDependent, DocumentFeatureExtractor
{

    public static final String PARAM_S_VERBS_FD_FILE = "TeacherVerbsFDFile";
    public static final String PARAM_USE_TOP_K_VERBS = "TopKVerbs";

    @ConfigurationParameter(name = PARAM_S_VERBS_FD_FILE, mandatory = true)
    private String studentVerbsFdFile;
        ...
        throws TextClassificationException
    {
        ...
        for (String topV : topVerbs) {
            if (features.contains(topV)) {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "S_feature_verb_Bool_" + topV, 1)));
            }
            else {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "S_feature_verb_Bool_" + topV, 0)));
            }
        }
      //Measure Cosine Similarity score between the top Verbs of the corpus and those of the lesson.
        ...
        featList.add(new Feature(GROUP_PREFIX+ "CosineSimilarityTopVerbs_S", cosineScore/nrOfLessonVerbs));
        return featList;
    }
    ...
    private List<Feature> countVerbHits(String wordListName, List<String> v)
            throws TextClassificationException
        {
            ...
            return Arrays.asList(new Feature(GROUP_PREFIX+ "S_feature_verb_ratio_" + wordListName,
        ...
        throws ResourceInitializationException
    {
        try {
            ...
            trainingFD.load(new File(studentVerbsFdFile));
    ...
    public List<Class<? extends MetaCollector>> getMetaCollectorClasses()
    {
        ...
        metaCollectorClasses.add(S_VerbsMetaCollector.class);
