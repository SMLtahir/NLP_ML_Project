...
/**
 * Feature extractor based on the frequency of top-occurring nouns in teacher utterances
 * 
 * @author Tahir Sousa
    ...
    implements MetaDependent, DocumentFeatureExtractor
{
        ...
        throws TextClassificationException
    {
        ...
        for (String topNn : topNouns) {
            if (features.contains(topNn)) {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_nn_Bool_" + topNn, 1)));
            }
            else {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_nn_Bool_" + topNn, 0)));
            }
        }
        ...
        featList.add(new Feature(GROUP_PREFIX+ "CosineSimilarityTopNouns_T", cosineScore/nrOfLessonNouns));
        return featList;
    }
    ...
    private List<Feature> countNounHits(String wordListName, List<String> nn)
            throws TextClassificationException
        {
            ...
            return Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_nn_ratio_" + wordListName,
