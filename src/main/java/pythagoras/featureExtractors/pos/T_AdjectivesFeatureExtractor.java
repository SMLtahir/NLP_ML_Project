...
import pythagoras.featureExtractorUtils.T_AdjectivesMetaCollector;
...
/**
 * Feature extractor based on the frequency of top-occurring adjectives in teacher utterances
 * 
 * @author Tahir Sousa
 * @version last updated: Apr 22, 2014 [Sousa]
 */
public class T_AdjectivesFeatureExtractor
    extends FeatureExtractorResource_ImplBase
    implements MetaDependent, DocumentFeatureExtractor
{

    public static final String PARAM_T_ADJECTIVES_FD_FILE = "TeacherAdjectivesFdFile";
    public static final String PARAM_USE_TOP_K_ADJECTIVES = "TopKAdjectives";

    @ConfigurationParameter(name = PARAM_T_ADJECTIVES_FD_FILE, mandatory = true)
    private String teacherAdjectivesFdFile;
        ...
        throws TextClassificationException
    {
        ...
        for (String topAdj : topAdjectives) {
            if (features.contains(topAdj)) {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_adj_Bool_" + topAdj, 1)));
            }
            else {
                featList.addAll(Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_adj_Bool_" + topAdj, 0)));
            }
        }
        ...
        featList.add(new Feature(GROUP_PREFIX+ "CosineSimilarityTopAdjectives_T", cosineScore/nrOfLessonAdjectives));
        return featList;
    }
    ...
    private List<Feature> countAdverbHits(String wordListName, List<String> adj)
            throws TextClassificationException
        {
            ...
            return Arrays.asList(new Feature(GROUP_PREFIX+ "T_feature_adj_ratio_" + wordListName,
        ...
        throws ResourceInitializationException
    {
        try {
            ...
            trainingFD.load(new File(teacherAdjectivesFdFile));
    ...
    public List<Class<? extends MetaCollector>> getMetaCollectorClasses()
    {
        ...
        metaCollectorClasses.add(T_AdjectivesMetaCollector.class);
