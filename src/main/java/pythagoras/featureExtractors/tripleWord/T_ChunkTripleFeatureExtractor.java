...
import pythagoras.featureExtractorUtils.T_ChunkTripleMetaCollector;
...
/**
 * Feature extractor based on the frequency of lexical triples in teacher utterances only
 * 
 * @author Tahir Sousa
 * @version last updated: Apr 03, 2014 [Sousa]
 */
@TypeCapability(inputs = { "de.tudarmstadt.ukp.dkpro.core.api.syntax.type.chunk.Chunk" })
public class T_ChunkTripleFeatureExtractor
    extends FeatureExtractorResource_ImplBase
    implements DocumentFeatureExtractor, MetaDependent
{

    public static final String PARAM_CHUNK_TRIPLE_FD_FILE = "teacherChunkTripleFdFile";
    @ConfigurationParameter(name = PARAM_CHUNK_TRIPLE_FD_FILE, mandatory = true)
    private String teacherChunkTripleFdFile;
        ...
        throws TextClassificationException
    {
        ...
        if (!stopwords.isEmpty()) {
            triples = T_ChunkTripleMetaCollector.getTriples(jcas, chunkTripleLowerCase, stopwords);
        } else {
            triples = T_ChunkTripleMetaCollector.getTriples(jcas, chunkTripleLowerCase);        	
        }
        for (String featureTriple : tripleSet) {
            if (triples.contains(featureTriple)) {
                features.add(new Feature("T_lexicalTriple_" + featureTriple, 1));
            }
            else {
                features.add(new Feature("T_lexicalTriple_" + featureTriple, 0));
            }
        }
        ...
        throws ResourceInitializationException
    {
        ...
        try {
            ...
            trainingFD.load(new File(teacherChunkTripleFdFile));
        }
        catch (IOException e) {
    ...
    public List<Class<? extends MetaCollector>> getMetaCollectorClasses()
    {
        ...
        metaCollectorClasses.add(T_ChunkTripleMetaCollector.class);
