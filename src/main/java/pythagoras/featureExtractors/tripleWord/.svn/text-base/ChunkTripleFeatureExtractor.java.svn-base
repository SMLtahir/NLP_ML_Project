...
/**
 * Feature extractor based on the frequency of lexical triples in all utterances
 * 
 * @author Tahir Sousa
    ...
    implements DocumentFeatureExtractor, MetaDependent
{
    ...
    public static final String PARAM_CHUNK_TRIPLE_FD_FILE = "allChunkTripleFdFile";
    @ConfigurationParameter(name = PARAM_CHUNK_TRIPLE_FD_FILE, mandatory = true)
    private String allChunkTripleFdFile;

    public static final String PARAM_CHUNK_TRIPLE_THRESHOLD = "chunkTripleThreshold";
    @ConfigurationParameter(name = PARAM_CHUNK_TRIPLE_THRESHOLD, mandatory = false, defaultValue = "1")
        ...
        throws TextClassificationException
    {
        ...
        if (!stopwords.isEmpty()) {
            triples = ChunkTripleMetaCollector.getTriples(jcas, chunkTripleLowerCase, stopwords);
        } else {
            triples = ChunkTripleMetaCollector.getTriples(jcas, chunkTripleLowerCase);        	
        }
        ...
        throws ResourceInitializationException
    {
        ...
        try {
            ...
            trainingFD.load(new File(allChunkTripleFdFile));
        }
        catch (IOException e) {
