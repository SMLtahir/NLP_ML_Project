...
//    public static String[] COST_MATRIX = {"[0 0.11; 0.89 0]", 		//Relevanz
//			"[0 0.75; 0.25 0]", 									//Rez
//			"[0 0.55; 0.45 0]", 									//Gem
//			"[0 0.20; 0.80 0]", 									//Denk
//			"[0 0.40; 0.60 0]", 									//Koop
...
//			"[0 0.35; 0.65 0]"};									//OverallGoodBad
	...
	public static void main(String[] args) throws Exception {
		...
		String[] mainGroupIds = {"CSur_&&TB_", "CSur_&&LS_", "CSur_&&Ph_&&Phon_", "CSur_&&Disc_", "CSur_&&SE_", "CSur_&&CSyn_", "CSur_&&CNg_"};
		for(int id =0; id< mainGroupIds.length; id++)	{
			String mainGroupId = mainGroupIds[id];
			//Loop over each of the selected dimensions
			for	(int dimIndex = 2; dimIndex<= 2; dimIndex++)	{
				//InputFilePath = training-data.arff file containing DKProTCInstanceID
				String inputFileDir = "E:/Eclipse/DKPro_Workspace/Pythagoras/de.tudarmstadt.ukp.dkpro.lab/repository/Classification_"+ 
						DIMENSIONS[dimIndex];
				String inputFileName = "-training-data"+ mainGroupId;
//				String inputFileName = "-training-data";
				String inputFilePath = inputFileDir + inputFileName + ".arff";
				String outputFilePath = inputFileDir+ "_Results" + ".txt";
				...
				for (int fold = 1; fold<= initialData.numInstances() ; fold++)	{
					...
					if	(predictedClass == 0.0)	{
						predictedOutcome = DIMENSIONS[dimIndex]+"Bad";
						class0Predictions++;
						...
						predictedOutcome = DIMENSIONS[dimIndex]+"Good";
						class1Predictions++;
						if (predictedOutcome.equals(realOutcome))	{
							confusionMatrix[1][1]++;
						}
				...
				out.write("Results of leaving out\t"+ mainGroupId+ "\tfrom the feature groups for Dimension:\t"+ DIMENSIONS[dimIndex]);
